...
(outputs/<client>/monthly_ndvi_ndwi_ndmi.nc) and the already-fetched ERA5
monthly series (outputs/<client>/{ndwi,ndmi}_temperature_monthly.csv) - no
new network calls. Aggregates the clipped pixel grid into 5x5-pixel sectors
(partial sectors at the bbox edges), computes each sector's series +
dryness frequency/magnitude + correlation with temperature for BOTH NDWI
and NDMI, each over the full 5-year history and over just the last
...
Run:    python build_sector_explorer_data.py --client <slug>
Setup:  run after pilot_historical_analysis.py for the same client -
        reuses its output, does not download anything itself.
"""
import base64
import json
import math
...
def build():
    # Read thresholds here (not at import time) so a per-client override
    # applied by clients.apply_client_overrides() in __main__ - which runs
    # before build() but after this module is imported - is picked up.
    index_thresholds = {"NDWI": config.NDWI_DRY_THRESHOLD, "NDMI": config.NDMI_DRY_THRESHOLD}

    ds = open_cube(config.OUTPUT_DIR / "monthly_ndvi_ndwi_ndmi.nc")
    ...
    era5 = pd.read_csv(config.OUTPUT_DIR / "era5_temperature_monthly.csv", parse_dates=["month"])
    ...
    def build_index_payload(variable, threshold):
        ...
        plot_ts = pd.read_csv(config.OUTPUT_DIR / f"{variable.lower()}_temperature_monthly.csv", parse_dates=["month"])
